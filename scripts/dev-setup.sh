...
#
# Baseline: validate Node against engines.node, enable Corepack, frozen install, print the package
# manager and pnpm store path. Add repo-specific steps only when this repo grows beyond docs and
# policy files; keep them idempotent and never clobber an existing env file.
#
set -euo pipefail
