...
# (`pnpm worktree:new`). Enforces the invariant: no active worktree lives inside another checkout.
#
...
#
# PR checkout (e.g. `gh pr checkout` into a sibling worktree) is a deliberate extension point —
# add it here when a repo needs it; keep the external-sibling invariant.
#
set -euo pipefail
